...
namespace CryptoHelper
{
    public class SM2CryptoUtil
    {
        ...
        public static void GenerateKeyHex(out string pubkey, out string privkey)
        {
            // byte[] a=new byte[1];
            // byte[] b=new byte[1];
            //GenerateKey(out a, out new byte[] b);
            ...
            byte[] pubkeyByte = ((ECPublicKeyParameters)k.Public).Q.GetEncoded(false);
            byte[] privkeyByte = ((ECPrivateKeyParameters)k.Private).D.ToByteArray();

            pubkey = Hex.ToHexString(pubkeyByte);
            privkey = Hex.ToHexString(privkeyByte);
        }
